...
export function UpdateReserva() {
  ...
  useEffect(() => getReservaData(reservaId), []);

  const getReservaData = async (id) => {
    toast("Cargando reserva...", {
      progress: true,
      closeButton: false,
      closeOnClick: false,
      isLoading: true,
    ...
    const { recursosDisponibles, reserva } = JSON.parse(
      await socketClient.getReserva(String(id))
    );

    if (String(reserva.cocheras.precioPorDia).startsWith("0"))
      reserva.cocheras.precioPorDia = null;
    if (String(reserva.cocheras.precioTotal).startsWith("0"))
      reserva.cocheras.precioTotal = null;

    setData((old) => ({
      ...old,
      cocheras: {
        numeros: recursosDisponibles.cocheras.map((x) => x.nombre),
      },
      idRecursosDisponibles: recursosDisponibles.recursos,
    }));

    setearReserva(reserva);
  };

  const setearReserva = (reserva) => {
    const { general, cocheras, pagos } = reserva;
    setNombre(general.nombre);
    setLocalidad(general.localidad);
    setTelefono(general.telefono);
    setMail(general.mail);
    setCantPersonas(general.cantPersonas);
    setFechaIngreso(general.fechaIngreso);
    setFechaEgreso(general.fechaEgreso);
    setIdRecurso(general.idRecurso);
    setTipoRecurso(general.tipoRecurso);
    setComentario(general.comentario);
    setRecursoPrecioPorDia(general.precioPorDia);
    setRecursoPrecioTotal(general.precioTotal);
    setCocherasNumeros(cocheras.numeros);
    setCocherasPrecioPorDia(cocheras.precioPorDia);
    setCocherasPrecioTotal(cocheras.precioTotal);
    setPagos(pagos);
  };
  ...
  const updateRecursosDisponibles = async (ingreso, egreso, tipoRecurso) => {
    ...
    const response = JSON.parse(
      await socketClient.getRecursosDisponiblesConReserva(
        ingreso,
        egreso,
        tipoRecurso,
        reservaId
  ...
  const confirmSaveReserva = async () => {
    ...
    await socketClient.updateReserva(payload);
  ...
  const confirmDeleteReserva = async () => {
    ...
    await socketClient.deleteReserva(reservaId);

    resetForm();

    toast("Reserva eliminada!", { type: "success" });
    navigate(-1);
  };
  ...
  return (
    <LocalizationProvider dateAdapter={AdapterMoment}>
      <div style={{ ...HomePageStyle.container, paddingBottom: "80px" }}>
        <Typography sx={ReservaRapidaStyle.textField}>
          Editando Reserva {reservaId}, en la {tipoRecurso} {idRecurso}
        </Typography>
        <Accordion>
